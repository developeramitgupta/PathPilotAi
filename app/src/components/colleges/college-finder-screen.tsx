...
export function CollegeFinderScreen() {
  ...
  const [input, setInput] = useState<CollegeFinderInput>(() => ({
    annualBudget: budgetByBand[profile?.studyBudget ?? "medium"], state: stateByCity[profile?.city ?? ""] ?? "All India", city: "", ownership: "any", hostel: "preferred", minPlacementRate: 0, placementPriority: 4, branch: "Any programme", scholarshipNeed: false, boardPercentile: 82, cultureTags: ["tech-clubs"],
  }));
      ...
      <div className="mt-7 grid items-start gap-6 xl:grid-cols-[310px_1fr]">
        <Card className="p-5 xl:sticky xl:top-24">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <FieldLabel htmlFor="minimum-placement" label="Minimum published placement rate" hint={input.minPlacementRate ? `${input.minPlacementRate}%+` : "Any"}><Select id="minimum-placement" value={input.minPlacementRate} onChange={(event) => setInput({ ...input, minPlacementRate: Number(event.target.value) })}><option value={0}>Any published rate</option><option value={50}>50% or higher</option><option value={60}>60% or higher</option><option value={70}>70% or higher</option><option value={80}>80% or higher</option></Select></FieldLabel>
